# CLAUDE.md — Cortex Development Guide

## Project Overview

Cortex is an AI-powered adaptive learning platform for the Dallas AI Hackathon 2026. It uses a knowledge graph canvas (React Flow), ML-based knowledge decay prediction (XGBoost + Ebbinghaus curves), Google Gemini AI for personalized learning, and a YouTube snippet pipeline for targeted video learning.
...
3. Token is stored in `localStorage` as `cognipath_token (legacy key name, kept for compatibility)`
...
9. Demo user: `alex@cortex.dev` / `demo1234` (created by seed script)
...
- `MONGO_DB_NAME` — Database name (default: `cortex`)
