...
async def generate_summary(req: SummaryRequest, current_user: User = Depends(get_current_user)):
    """Generate a summary of the given topic, personalized with past learning context."""
    try:
        context, related_topics = _get_related_context(str(current_user.id), req.topic)
